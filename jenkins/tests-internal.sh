set -e;
echo "sudo docker run -d $DOCKER_REGISTRY/$JOB_NAME:$BUILD_NUMBER bash -c 'npm test'";
ID=`sudo docker run -d $DOCKER_REGISTRY/$JOB_NAME:$BUILD_NUMBER bash -c 'npm test'`
